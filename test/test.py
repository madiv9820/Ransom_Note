...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        ransomNote: str = testcase['input']['ransomNote']
        magazine: str = testcase['input']['magazine']
        ...
        print(f'📝 Ransom Note  : {ransomNote}')
        print(f'📓 Magazine : {magazine}')
        print('⏳ Starting soon...')
        time.sleep(self.DISPLAY_DELAY_SECONDS)

        try:
            actual_output: bool = self._run_case(ransomNote=ransomNote, magazine=magazine)
        except TimeoutError:
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, ransomNote: str, magazine: str) -> bool:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        return self.__solution.canConstruct(ransomNote=ransomNote, magazine=magazine)


class RansomNoteTextTestResult(unittest.TextTestResult):
    ...
    def stopTestRun(self):
        ...
        print('\n' + '🧾' + '=' * 58)
        print('📚 Ransom Note Final Summary')
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=RansomNoteTextTestResult)
    )
